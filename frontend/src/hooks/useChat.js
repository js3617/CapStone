...
function useChat() {
    ...
    useEffect(() => {
        addMessage('bot', "현재 느끼고 있는 증상을 알려주세요! ex)'두통이 있어요', '열이 나요', '복통이 심해요'와 같이 입력해주세요.");
    }, []);
    ...
    const handleSendMessage = async () => {
        ...
        const symptom = extractSymptom(message) || '증상 불명';
    ...
    const extractSymptom = (input) => {
        const symptoms = ['두통', '발열', '복통', '기침', '피로'];
        return symptoms.find(symptom => input.includes(symptom)) || null;
    };

    const createPrompt = (symptom) => {
        return `사람의 현재 증상이 기록된 [${symptom}]과 관련된 병명과 원인을 출력하고, 병명이 기록된 [${symptom}]에 대한 증상을 완화할 수 있는 일반의약품 종류도 함께 출력해줘
                병명: 해당 문제에 대한
                ...
                이 형식을 준수하여 3개 이상의 결과를 제공하십시오.
                각 결과는 반드시 줄 바꿈(\n)으로 구분되어야 합니다.`;
